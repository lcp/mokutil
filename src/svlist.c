...
#define SVLIST_BASIC_SIZE (sizeof(svlist_t) + sizeof(svnode_t))
...
print_help ()
{
	...
	printf ("  --export <file>\tExport the list in binary form\n");
	...
	printf ("  --cert <certificate>\t\tVerify with this certificate\n");
	...
	printf ("\n");
	printf ("Show the content of a binary list\n");
	printf (" svlist --bin list.bin");
	printf ("\n");
...
static int
parse_line (char *line, const int64_t length, svlist_t **list)
{
	svlist_t *lptr = NULL;
	svnode_t node;
	const char *delim = ",";
	char *str = NULL, *token, c;
	int64_t i, j, k, value;
	...
	/* Count commas to estimate the length of the list */
	count = 0;
	for (i = 0; i < length; i++) {
		if (str[i] == ',')
			count++;
	...
	lptr->size = (uint16_t)list_size;

	lptr->magic = SVLIST_MAGIC;
	...
	/*   Copy the first SIGNER_MAX_SIZE - 1 characters */
	strncpy (lptr->signer, token, SIGNER_MAX_SIZE - 1);
	lptr->signer[SIGNER_MAX_SIZE - 1] = '\0';

	/*   Remove the trailing whitespaces and check the string */
	for (i = SIGNER_MAX_SIZE - 2; i >= 0; i--) {
		c = lptr->signer[i];
	...
	for (; i >= 0; i--) {
		c = lptr->signer[i];
		if (!isalnum(c) && c != ' ') {
			fprintf (stderr, "Invalid character found in \"%s\"\n",
					 lptr->signer);
			goto error;
		}
	}
...
static int
verify_svlist_data (const void *buffer, const uint64_t size)
{
	void *ptr = (void *)buffer;
	svlist_t *list;
	uint64_t offset = 0, diff;
	uint16_t list_size;

	while (offset < size) {
		diff = size - offset;

		if (diff < SVLIST_BASIC_SIZE)
			return -1;

		list = (svlist_t *)ptr;

		if (list->magic != SVLIST_MAGIC)
			return -1;

		list_size = list->size;

		if (list_size > diff || list_size < SVLIST_BASIC_SIZE ||
		...
		uint64_t *var_size)
{
	...
	if (verify_svlist_data (content, size) < 0) {
...
static svlist_t *
match_signer (const void *data, const uint64_t data_size, const char *signer)
{
	svlist_t *list;
	uint64_t skip = 0;
	...
	while (skip < data_size) {
		...
		if (strncmp (list->signer, signer, SIGNER_MAX_SIZE) == 0)
			return list;
		skip += list->size;
	}
	return NULL;
		...
		const void *req, const uint64_t req_size)
{
	...
	while (off_req < req_size) {
		...
		var_lptr = match_signer (var, var_size, req_lptr->signer);
		if (var_lptr == NULL)
			return 0;
...
static int
copy_data (const void *content, const off_t size, void **data,
	   uint64_t *data_size)
{
	void *ptr;
			...
			const uint64_t sig_size)
{
	...
	if (!efi_variables_supported ()) {
		fprintf (stderr, "EFI variables not supported\n");
		return -1;
	}
...
print_var (const void *var, const uint64_t var_size)
{
	...
	/* The input is supposed to be checked and verified. Skip the check and
	...
	while (offset < var_size) {
		...
		printf ("%s", list->signer);
		for (i = 0; i < list_n; i++) {
			node = list->nodes + i;
			printf (", %d, %d", node->dv, node->sv);
		}
		putchar ('\n');
...
main (int argc, char *argv[])
{
	...
	/* Print the input file if there is no other operation */
	if ((command ^ OPT_BIN_INPUT) == 0 || (command ^ OPT_TXT_INPUT) == 0)
		command |= OPT_SHOW;
	...
	if (command & OPT_VERIFY) {
		if (verify_sig (req, req_size, sig, sig_size, cert,
				cert_size) < 0) {
